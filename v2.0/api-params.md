...
> 温馨提示：如果对过滤器配置了白名单，必选参数最终会自动切换为可选参数，即```require = false```，详细请参考[白名单配置](v2.0/filter?id=%E7%99%BD%E5%90%8D%E5%8D%95%E9%85%8D%E7%BD%AE)。   
...
公共的配置选项，除了上面的类型、参数名称、是否必须、默认值，还有说明描述、数据来源。下面分别简单说明。  
 ...
 + **参数名称 name**  
 接口参数名称，即客户端需要传递的参数名称。与PHP变量规则一样，以下划线或字母开头。**此选项必须提供，否则会提示错误：** ```服务器运行错误：参数规则缺少name```。   
  
 + **是否必须require**  
 为TRUE时，表示此参数为必须值；为FALSE时，表示此参数为可选。未指定时，默认为FALSE。  

> 特别注意：require为true时表示是指客户端必须传此参数，当客户端什么都不传（即为null）会出错，而当客户端传空字符串时则满足要求，例如：```username=```。尤其当类型为int类型，且客户端传递了空字符串时，最终接口内部接收到的将是隐式类型转换后的结果，即数字0。可以结合min配置项限制客户端传递的最小值和有效范围。  
